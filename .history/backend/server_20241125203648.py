...
async def detect_species(frame_data: Frame):
    ...
    try:
        ...
        print(f"Site ID: {frame_data.site_id}")
        ...
        print("Detection passed confidence threshold")
        ...
        # Get frame dimensions from the actual frame
        ...
        print(f"Returning detection: {prediction['species']} with confidence {prediction['confidence']:.2f}")
        return {'detections': [detection]}
        
    except Exception as e:
        print(f"Error in detect_species:")
        import traceback
        traceback.print_exc()
