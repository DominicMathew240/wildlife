...
async def detect_species(frame_data: Frame):
    ...
    try:
        ...
        if frame is None:
            print("Failed to decode frame") # Debug log
            raise HTTPException(status_code=400, detail="Invalid frame data")
        
        print("Frame decoded successfully") # Debug log
        
        # Convert to PIL Image
        image = Image.fromarray(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))
        
        print("Running species classification...") # Debug log
        # Get prediction
        ...
        print(f"Image saved at: {image_path}") # Debug log
        ...
        print(f"Sending detection response: {detection}") # Debug log
        return {'detections': [detection]}
        
    except Exception as e:
        print(f"Error in detect_species: {str(e)}") # Debug log
