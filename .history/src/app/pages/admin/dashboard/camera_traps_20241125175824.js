...
export default function CameraTraps() {
    ...
    // Process video frames for species detection
    const processFrames = async () => {
        ...
        try {
            const response = await axios.post('http://localhost:8000/api/detect', { frame });
            if (response.data.detections?.length > 0) {
                ...
                if (detection.confidence >= 0.7) {
                    ...
                    // Store detection in database (you'll need to implement this API)
                    await axios.post('/api/store-detection', newDetection);
                }
            }
        } catch (err) {
